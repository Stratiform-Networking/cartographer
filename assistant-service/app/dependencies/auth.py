...
async def verify_token_with_auth_service(token: str) -> AuthenticatedUser | None:
    ...
    try:
        async with httpx.AsyncClient(timeout=10.0) as client:
            ...
            return AuthenticatedUser(
                user_id=data["user_id"], username=data["username"], role=UserRole(data["role"])
            )
...
def require_auth_with_rate_limit(limit: int, endpoint: str):
    ...
    async def _dependency(user: AuthenticatedUser = Depends(require_auth)) -> AuthenticatedUser:
