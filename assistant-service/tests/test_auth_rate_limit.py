...
class TestAuthenticatedUser:
    ...
    def test_authenticated_user_creation(self):
        ...
        user = AuthenticatedUser(user_id="test-123", username="testuser", role=UserRole.MEMBER)
...
class TestServiceTokenVerification:
    ...
    def test_valid_service_token(self):
        ...
        payload = {"service": True, "sub": "metrics-service", "username": "metrics"}
    ...
    def test_expired_service_token(self):
        ...
        payload = {"service": True, "sub": "service", "exp": datetime.utcnow() - timedelta(hours=1)}
        token = jwt.encode(payload, settings.jwt_secret, algorithm=settings.jwt_algorithm)
...
class TestRateLimitHelpers:
    ...
    def test_is_role_exempt_true(self):
        ...
        with patch.object(
            type(settings), "rate_limit_exempt_roles", property(lambda self: {"admin", "owner"})
        ):
    ...
    def test_is_role_exempt_false(self):
        ...
        with patch.object(
            type(settings), "rate_limit_exempt_roles", property(lambda self: {"admin"})
        ):
...
class TestGetUserLimit:
    ...
    async def test_exempt_role_no_database(self):
        ...
        from app.services.rate_limit import UNLIMITED_LIMIT, get_user_limit
...
class TestCheckRateLimit:
    ...
    async def test_unlimited_user_bypasses_check(self):
        ...
        from app.services.rate_limit import UNLIMITED_LIMIT, check_rate_limit

        with patch(
            "app.services.rate_limit.get_user_limit", AsyncMock(return_value=UNLIMITED_LIMIT)
        ):
            # Should not raise
...
class TestGetRateLimitStatus:
    ...
    async def test_unlimited_user_status(self):
        ...
        from app.services.rate_limit import UNLIMITED_LIMIT, get_rate_limit_status
...
class TestRequireAuthWithRateLimit:
    ...
    async def test_creates_dependency(self):
        ...
        from app.dependencies.auth import AuthenticatedUser, UserRole, require_auth_with_rate_limit
...
class TestGetCurrentUserEdgeCases:
    ...
    async def test_falls_back_to_auth_service(self):
        ...
        credentials = HTTPAuthorizationCredentials(
            scheme="Bearer", credentials="not-a-service-token"
        )

        with patch(
            "app.dependencies.auth.verify_token_with_auth_service", AsyncMock(return_value=None)
        ):
            user = await get_current_user(credentials, None)
