...
class TestUserBase:
    ...
    def test_username_validation_start_with_letter(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            UserBase(
                username="1invalid", first_name="Test", last_name="User", email="test@example.com"
            )
...
class TestLoginModels:
    ...
    def test_login_response(self):
        ...
        response = LoginResponse(access_token="token123", expires_in=3600, user=user)
...
class TestTokenPayload:
    ...
    def test_token_payload(self):
        ...
        payload = TokenPayload(
            sub="user-123", username="testuser", role=UserRole.OWNER, exp=now, iat=now
        )
...
class TestSetupStatus:
    ...
    def test_setup_status(self):
        ...
        status = SetupStatus(is_setup_complete=True, owner_exists=True, total_users=3)
...
class TestSessionInfo:
    ...
    def test_session_info(self):
        ...
        session = SessionInfo(user=user, permissions=["read:map", "read:health"])
...
class TestInviteCreate:
    ...
    def test_valid_invite_create(self):
        ...
        invite = InviteCreate(email="invite@test.com", role=UserRole.MEMBER)
        assert invite.email == "invite@test.com"

    def test_cannot_invite_owner(self):
        """Should reject owner role"""
        with pytest.raises(ValidationError) as exc_info:
            InviteCreate(email="invite@test.com", role=UserRole.OWNER)
