...
class Network(Base):
    ...
    agent_key: Mapped[Optional[str]] = mapped_column(String(64), unique=True, nullable=True)
...
class NetworkPermission(Base):
    ...
    user_id: Mapped[str] = mapped_column(UUID(as_uuid=False), index=True)
    role: Mapped[PermissionRole] = mapped_column(SQLEnum(PermissionRole, name="permission_role"))
