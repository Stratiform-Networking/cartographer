...
class TestPostHogHelpers:
    ...
    async def test_dispatch_tracks_exception_path(self):
        ...
        middleware = UsageTrackingMiddleware(app, service_name="backend")
        middleware._running = True
    ...
    async def test_flush_buffer_returns_early_when_batch_size_zero(self):
        ...
        with patch.object(middleware._settings, "usage_batch_size", 0):
            await middleware._flush_buffer()
    ...
    async def test_shutdown_flushes_pending_records(self):
        ...
        with patch.object(middleware, "_flush_buffer", side_effect=fake_flush) as mock_flush:
            await middleware.shutdown()
