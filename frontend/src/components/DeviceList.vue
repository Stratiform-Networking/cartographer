<template>
  <div class="h-full flex flex-col">
    <!-- Header -->
    <div
      class="flex items-center gap-2 px-4 py-3 border-b border-slate-200 dark:border-slate-800/80 bg-slate-50 dark:bg-slate-950/50"
    >
      <svg
      ...
      <span class="ml-auto text-xs text-slate-500 dark:text-slate-400 tabular-nums">{{
        filtered.length
      }}</span>
    </div>
    ...
    <div class="flex-1 overflow-auto">
      ...
      <div v-if="filtered.length === 0 && query" class="p-6 text-center">
        <svg
          xmlns="http://www.w3.org/2000/svg"
          class="h-10 w-10 mx-auto mb-2 text-slate-300 dark:text-slate-700"
          fill="none"
          viewBox="0 0 24 24"
          stroke="currentColor"
          stroke-width="1"
