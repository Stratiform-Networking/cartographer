<template>
	<div class="min-h-screen bg-white dark:bg-slate-950 flex items-center justify-center p-6 transition-colors relative overflow-hidden">
		<!-- Background effects -->
		<div class="fixed inset-0 bg-mesh-gradient-light dark:bg-mesh-gradient opacity-50 dark:opacity-50 pointer-events-none" />
		<div class="fixed top-0 right-0 w-[600px] h-[600px] bg-cyan-400/5 dark:bg-cyan-500/5 rounded-full blur-3xl pointer-events-none" />
		<div class="fixed bottom-0 left-0 w-[600px] h-[600px] bg-blue-400/5 dark:bg-blue-600/5 rounded-full blur-3xl pointer-events-none" />

		<div class="relative z-10 w-full max-w-md">
			<!-- Logo/Header -->
			<router-link to="/" class="flex items-center justify-center gap-3 mb-10">
				<div class="w-12 h-12 bg-gradient-to-br from-cyan-500 to-blue-600 rounded-2xl flex items-center justify-center shadow-lg shadow-cyan-500/20">
				...
				<span class="text-2xl font-display font-bold text-slate-900 dark:text-white">Cartographer</span>
			</router-link>

			<!-- Login Card -->
			<div class="glass-card p-8 border-gradient">
				<div class="text-center mb-8">
					<h1 class="text-2xl font-display font-bold text-slate-900 dark:text-white mb-2">Welcome back</h1>
				...
				<form @submit.prevent="onSubmit" class="space-y-5">
					...
					>
						<div v-if="errorMessage" class="p-4 bg-red-50 dark:bg-red-500/10 border border-red-200 dark:border-red-500/30 rounded-xl">
				...
				<div class="mt-6 text-center">
					<p class="text-slate-500 dark:text-slate-400">
						...
						<a href="/signup" class="text-cyan-600 dark:text-cyan-400 hover:text-cyan-500 dark:hover:text-cyan-300 transition-colors font-medium">
							Sign up
						</a>
					</p>
				</div>
