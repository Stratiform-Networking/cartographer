<template>
  ...
  <div
    v-if="!isNetworkMode && authLoading"
  ...
  <div v-else class="h-screen flex flex-col bg-white dark:bg-slate-950 relative">
    ...
    <div
      class="absolute inset-0 bg-mesh-gradient-light dark:bg-mesh-gradient opacity-30 dark:opacity-20 pointer-events-none"
    />

    <div class="flex flex-1 min-h-0 relative z-10">
      <aside
        class="w-80 border-r border-slate-200 dark:border-slate-800/80 bg-white/95 dark:bg-slate-900/95 backdrop-blur-sm"
      >
        <DeviceList
          v-if="parsed"
          :root="parsed.root"
      ...
      <main class="flex-1 p-3 relative bg-slate-50/50 dark:bg-slate-950/50">
        <!-- Unified top toolbar -->
        <div
          class="absolute top-3 left-3 right-3 z-10 flex items-center justify-between pointer-events-none"
        >
          <!-- Left: Navigation controls (horizontal) -->
          <div
            class="flex items-center h-8 bg-white/90 dark:bg-slate-900/90 backdrop-blur-lg border border-slate-200/80 dark:border-slate-700/50 rounded-lg shadow-sm px-1 pointer-events-auto"
          >
            <button
          ...
          <div class="flex items-center gap-1.5 pointer-events-auto">
            ...
            <!-- Mode toggle -->
            <div
              class="flex items-center h-8 bg-white/90 dark:bg-slate-900/90 backdrop-blur-lg border border-slate-200/80 dark:border-slate-700/50 rounded-lg shadow-sm px-0.5"
            >
              <button
              ...
              <button
                ...
                :title="
                  effectiveCanWrite
                    ...
                    : 'Edit mode requires write permissions'
                "
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
        ...
        >
          ...
          >
            <div
              class="bg-white/90 dark:bg-slate-900/90 backdrop-blur-lg border border-slate-200/80 dark:border-slate-700/50 rounded-xl shadow-lg p-2"
            >
              <div class="flex items-center gap-3">
                <!-- Node type selector -->
                <div class="flex items-center gap-1.5">
                  <span
                    class="text-[10px] uppercase tracking-wider text-slate-500 dark:text-slate-400 font-medium"
                    >Type</span
                  >
                  <select
        ...
        <div class="w-full h-full">
          ...
          <div
            v-else
            class="h-full rounded-xl border border-dashed border-slate-300 dark:border-slate-700 flex items-center justify-center text-slate-500 dark:text-slate-400 bg-white/50 dark:bg-slate-900/50"
          >
            No map loaded yet.
          </div>
        </div>
      ...
      >
        ...
        <div class="flex-1 overflow-auto p-3">
          ...
          <div v-if="historyFilter === 'selected'">
            <div v-if="selectedNode && selectedNode.history?.length" class="space-y-3">
              <div class="mb-3 p-2 rounded-lg bg-slate-100 dark:bg-slate-800/50">
                ...
                <div class="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
                  ...
                  {{ selectedNode.history?.length || 0 }} changes
                </div>
              </div>
              <div
              ...
              >
                <div
                  class="absolute -left-1.5 top-0 w-3 h-3 rounded-full bg-amber-400 dark:bg-amber-500 border-2 border-white dark:border-slate-900"
                ></div>
            ...
            <div
              v-else-if="selectedNode && !selectedNode.history?.length"
          ...
          <div v-else>
            <div v-if="allNodesHistory.length" class="space-y-3">
              ...
              >
                <div
                  class="absolute left-0 top-1.5 w-3 h-3 rounded-full border-2 border-white dark:border-slate-900"
                  :class="getNodeRoleColor(item.role)"
                ></div>
                <div class="flex items-center gap-2 mb-0.5">
                  <span
                    class="text-xs font-medium text-slate-700 dark:text-slate-300 truncate max-w-48"
                    >{{ item.nodeName }}</span
                  >
    ...
    >
      ...
      >
        <div
          ...
          :key="idx"
          class="whitespace-pre-wrap text-slate-700 dark:text-slate-300"
        >
    ...
    >
      <div v-if="showAssistant" class="fixed right-0 top-0 bottom-0 w-[420px] z-50 shadow-2xl">
  ...
  async (root) => {
    if (root && props.networkId) {
      ...
      if (ips.length > 0) {
        ...
        console.log(
          `[Health] Registered ${ips.length} device IPs for monitoring (network ${props.networkId})`
        );
      }
...
onMounted(async () => {
  ...
  if (parsed.value?.root && !hasRegisteredDevices && props.networkId) {
    ...
    if (ips.length > 0) {
      ...
      hasRegisteredDevices = true;
      console.log(
        `[Health] Registered ${ips.length} device IPs for monitoring (on mount, network ${props.networkId})`
      );
    }
...
async function onToggleNodeMonitoring(nodeId: string, enabled: boolean) {
  ...
  if (node) {
    ...
    const ips = getMonitoredDeviceIPs(parsed.value.root);
    console.log(
      `[Health] ${enabled ? 'Enabling' : 'Disabling'} notifications for ${node.name || nodeId} (IP: ${node.ip})`
    );
...
function onChangeParent() {
  ...
  updateNodeVersion(node, [
    `Parent connection changed from "${oldParent || 'root'}" to "${connectParent.value || 'root'}"`,
  ]);
...
function onChangeConnectionSpeed() {
  ...
  // Track the version change
  updateNodeVersion(node, [
    `Connection speed changed from "${oldSpeed || 'none'}" to "${speed || 'none'}"`,
  ]);
...
function onChangeIp() {
  ...
  // Track the version change
  updateNodeVersion(node, [
    `IP address changed from "${oldIp || 'none'}" to "${(node as any).ip || 'none'}"`,
  ]);
...
function onChangeHostname() {
  ...
  // Track the version change
  updateNodeVersion(node, [
    `Hostname changed from "${oldHostname || 'none'}" to "${(node as any).hostname || 'none'}"`,
  ]);
...
function onRemoveNode() {
  ...
  walkAll(root, (n) => {
    if ((n as any).parentId === nodeId && n.id !== nodeId) {
      ...
      updateNodeVersion(n, [
        `Parent reassigned to root (previous parent "${nodeName}" was removed)`,
      ]);
    }
  });
