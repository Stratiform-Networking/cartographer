  ...
  <div class="w-full h-full relative">
    ...
    <div class="absolute inset-0 rounded-lg overflow-hidden pointer-events-none network-map-bg">
      ...
      <div
        ...
        style="
          background: linear-gradient(160deg, #05182f 0%, #020e20 50%, #03172d 100%);
          opacity: 0.5;
        "
      ></div>
      ...
      <div
        class="absolute inset-0 opacity-30 dark:opacity-15"
        style="
          background: radial-gradient(
    ...
    <svg
      ref="svgRef"
      class="relative w-full h-full rounded-lg border border-slate-200/60 dark:border-slate-800/60"
    ></svg>
  </div>
</template>
...
function render() {
  ...
  if (typeof (data as any).fx !== 'number' || typeof (data as any).fy !== 'number') {
    (data as any).fx = router.x;
    (data as any).fy = router.y;
    updatePosition(router.id, router.x, router.y);
  }
  nodes.push(router);
  ...
  for (let depth = 1; depth <= maxDepth; depth++) {
    ...
    nodesAtDepth.forEach((c, idx) => {
      ...
      if (typeof (c as any).fx !== 'number' || typeof (c as any).fy !== 'number') {
        (c as any).fx = dev.x;
        (c as any).fy = dev.y;
        updatePosition(c.id, dev.x, dev.y);
      }
      nodes.push(dev);
    });
  }
...
function zoomIn() {
  if (!svgRef.value || !currentZoom) return;
  const svg = d3.select(svgRef.value);
  svg.transition().duration(300).call(currentZoom.scaleBy, 1.4);
}
...
function resetView() {
  if (!svgRef.value || !currentZoom) return;
  const svg = d3.select(svgRef.value);
  svg.transition().duration(500).call(currentZoom.transform, d3.zoomIdentity.translate(24, 24));
}
