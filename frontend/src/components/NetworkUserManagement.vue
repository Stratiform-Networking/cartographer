      ...
      >
          ...
          <div class="flex items-center gap-2">
            ...
            <div>
              ...
              <p class="text-xs text-slate-500 dark:text-slate-400">
                Manage who has access to this network
              </p>
            </div>
          </div>
          <button
        ...
        >
          ...
          >
            Network Members
            <span
              v-if="networkMembers.length > 0"
              class="ml-1.5 px-1.5 py-0.5 text-xs bg-slate-200 dark:bg-slate-700 text-slate-600 dark:text-slate-400 rounded-full tabular-nums"
            >
          ...
          >
            ...
            <span
              ...
              class="ml-1.5 px-1.5 py-0.5 text-xs bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400 rounded-full tabular-nums"
            >
              ...
              <div class="space-y-2">
                ...
                >
                  <div class="flex items-center gap-3">
                    <div
                      class="w-8 h-8 rounded-lg flex items-center justify-center text-white text-sm font-medium bg-gradient-to-br from-amber-500 to-orange-600"
                    >
                  ...
                  <div class="text-xs text-slate-400 dark:text-slate-500 italic">Network owner</div>
                </div>
                ...
                >
                  <div class="flex items-center gap-3">
                    <div
                      class="w-8 h-8 rounded-lg flex items-center justify-center text-white text-sm font-medium bg-gradient-to-br from-cyan-500 to-blue-600"
                    >
                    ...
                    <div>
                      <div class="flex items-center gap-2">
                        ...
                        <span
                          :class="getRoleBadgeClass(member.permission.role)"
                  ...
                  <div class="flex items-center gap-1">
                    ...
                    <select
                      ...
                      @change="
                        onChangeRole(
                          member,
                          ($event.target as HTMLSelectElement).value as 'viewer' | 'editor'
                        )
                      "
                ...
                <!-- Empty State -->
                <div
                  v-if="networkMembers.length === 0 && networkOwner"
            ...
            <template v-else-if="activeTab === 'add'">
              <div class="space-y-2">
                ...
                >
                  <div class="flex items-center gap-3">
                    <div
                      class="w-8 h-8 rounded-lg flex items-center justify-center text-white text-sm font-medium bg-gradient-to-br from-slate-400 to-slate-600"
                    >
        ...
        <div class="p-4">
          <p class="text-sm text-slate-600 dark:text-slate-400 mb-4">
            ...
            <strong class="text-slate-800 dark:text-slate-200"
              >{{ removingMember.user.first_name }} {{ removingMember.user.last_name }}</strong
            >
            from this network? They will no longer be able to access it.
          </p>
...
const availableUsers = computed(() => {
  const existingUserIds = new Set([props.ownerId, ...permissions.value.map((p) => p.user_id)]);
...
async function onChangeRole(
  member: { user: User; permission: NetworkPermission },
  newRole: NetworkPermissionRole
) {
...
async function onRemoveMember() {
  ...
  try {
    ...
    permissions.value = permissions.value.filter(
      (p) => p.id !== removingMember.value!.permission.id
    );
