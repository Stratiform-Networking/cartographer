<template>
  ...
  >
    <!-- Header -->
    <div
      class="flex items-center justify-between px-4 py-3 border-b border-slate-200 dark:border-slate-800/80 bg-slate-50 dark:bg-slate-950/50"
    >
    ...
    <div class="flex-1 overflow-auto">
      ...
      <div
        ...
        :class="
          monitoringEnabled
            ? statusBannerClass
            : 'bg-slate-100 dark:bg-slate-950/50 text-slate-600 dark:text-slate-400'
        "
      >
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-2">
            ...
            <span class="font-medium text-sm">{{
              monitoringEnabled ? statusLabel : 'Monitoring Disabled'
            }}</span>
          </div>
          <button
      ...
      <div
        v-if="monitoringEnabled && loading"
        class="p-8 flex flex-col items-center justify-center text-slate-500 dark:text-slate-400"
      >
        <svg
      ...
      <div
        v-else-if="monitoringEnabled && !metrics && !error && node?.ip"
        class="p-8 flex flex-col items-center justify-center text-slate-500 dark:text-slate-400"
      >
        <svg
          class="animate-spin h-8 w-8 mb-3"
      ...
      <div v-if="monitoringEnabled && (metrics || isOffline)" class="p-4 space-y-4">
        ...
        <div
          ...
          class="bg-slate-100 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700/50/50 rounded-lg p-2 text-xs text-slate-500 dark:text-slate-400 text-center"
        >
          View only mode
        </div>
        ...
        <div
          v-if="isOffline"
          class="bg-red-50 dark:bg-red-900/15 border border-red-200 dark:border-red-800/50 rounded-lg p-3"
        >
          ...
          <div class="bg-slate-50 dark:bg-slate-800/40 rounded-lg p-3 space-y-2">
            <div class="grid grid-cols-2 gap-3">
              <MetricCard
                ...
                :sublabel="
                  metrics.ping.min_latency_ms && metrics.ping.max_latency_ms
            ...
            <div
              v-if="metrics.ping.jitter_ms != null"
              class="pt-2 border-t border-slate-200 dark:border-slate-700/50"
            >
              <div class="flex items-center justify-between">
                <span class="text-xs text-slate-500 dark:text-slate-400">Jitter</span>
                <span class="text-sm font-medium text-slate-700 dark:text-slate-300"
                  >{{ metrics.ping.jitter_ms.toFixed(2) }} ms</span
                >
              </div>
            </div>
          </div>
        ...
        <section
          v-if="
            monitoringEnabled &&
            (metrics?.uptime_percent_24h != null || metrics?.check_history?.length)
          "
        >
          <h3
            class="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-2 flex items-center gap-1.5"
          ...
          <div class="bg-slate-50 dark:bg-slate-800/40 rounded-lg p-3 space-y-3">
            ...
            <div class="space-y-2">
              <div class="flex items-center justify-between">
                ...
                <span
                  ...
                  :class="getUptimeColor(metrics?.uptime_percent_24h || 0)"
                >
            ...
            <div
              v-if="metrics?.avg_latency_24h_ms != null"
              class="flex items-center justify-between pt-2 border-t border-slate-200 dark:border-slate-700/50"
            >
              <span class="text-xs text-slate-500 dark:text-slate-400">Avg Latency (24h)</span>
              <span class="text-sm font-medium text-slate-700 dark:text-slate-300"
                >{{ metrics.avg_latency_24h_ms.toFixed(1) }} ms</span
              >
            </div>
            <div class="flex items-center justify-between text-xs">
              ...
              <span class="space-x-2">
                <span class="text-emerald-600 dark:text-emerald-400"
                  >{{ metrics?.checks_passed_24h || 0 }} passed</span
                >
                <span class="text-slate-400">/</span>
                <span class="text-red-600 dark:text-red-400"
                  >{{ metrics?.checks_failed_24h || 0 }} failed</span
                >
              </span>
            </div>
          </div>
        ...
        <section v-if="showLanPortsSection">
          ...
          <div v-show="lanPortsExpanded" class="space-y-3">
            ...
            <div
              v-if="!lanPortsConfig || lanPortsConfig.ports.length === 0"
              class="bg-slate-50 dark:bg-slate-800/40 rounded-lg p-4"
            >
              <div v-if="hasWritePermission && !showPortGridSetup" class="text-center">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  class="h-8 w-8 mx-auto mb-2 text-slate-400 dark:text-slate-500"
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                  stroke-width="1"
            ...
            <div v-else class="space-y-3">
              ...
              <div v-if="hasWritePermission" class="flex items-center justify-between">
                <span class="text-xs text-slate-500 dark:text-slate-400">
                  {{ lanPortsConfig.cols }} × {{ lanPortsConfig.rows }} grid ({{
                    lanPortsConfig.ports.length
                  }}
                  ports)
                </span>
                <div class="flex items-center gap-2">
                  ...
                  <button
                    @click="
                      showPortGridSetup = true;
                      portGridCols = lanPortsConfig.cols;
                      portGridRows = lanPortsConfig.rows;
                    "
              ...
              >
                {{
                  portEditMode
                    ? '⚙️ Edit Mode: Click ports to configure type, speed, status & PoE'
                    : '🔌 Click ports to set connections'
                }}
              </div>
              ...
              <div
                ...
                :class="
                  portEditMode
                    ? 'bg-amber-50 dark:bg-amber-900/10 ring-2 ring-amber-400/50'
              ...
              >
                <div
                  ...
                  :style="{
                    gridTemplateColumns: `repeat(${lanPortsConfig.cols}, minmax(32px, 1fr))`,
                  }"
                >
                  <div
                    ...
                    :class="[
                      ...
                      isDragOverPort(port) && !isDraggedPort(port)
                  ...
                  >
                    ...
                    >
                      ...
                      <span
                        ...
                        class="relative z-10 text-[10px] font-mono font-medium"
                        :class="getRj45TextClasses(port)"
                      >
                        {{ getPortLabel(port) }}
                      </span>
                      <span
                        v-else
                        class="text-slate-400 dark:text-slate-600 relative z-10 text-[10px] font-mono"
                        >✕</span
                      >
                    </div>
                    <!-- Port Visual - SFP/SFP+ -->
              ...
              <div class="flex flex-wrap gap-3 text-[10px] text-slate-500 dark:text-slate-400">
                <div class="flex items-center gap-1">
                  <div
                    class="w-3 h-3 rounded-sm bg-amber-200 dark:bg-amber-800 border border-amber-400 dark:border-amber-600 relative mb-1"
                  >
                    <div
                ...
                <div class="flex items-center gap-1">
                  <div
                    class="w-4 h-2.5 rounded bg-cyan-200 dark:bg-cyan-800 border border-cyan-400 dark:border-cyan-600 relative"
                  >
                    <div
                      class="absolute bottom-0.5 left-1/2 -translate-x-1/2 w-2 h-0.5 bg-cyan-600 dark:bg-cyan-400 opacity-30 rounded-full"
                    ></div>
                  </div>
                  <span>SFP/SFP+</span>
                </div>
                <div class="flex items-center gap-1">
                  <div
                    class="w-3 h-3 rounded bg-slate-300 dark:bg-slate-700 border border-slate-400 dark:border-slate-700"
                  ></div>
                  <span>Unused</span>
                </div>
                <div class="flex items-center gap-1">
                  <div
                    class="w-3 h-3 rounded bg-slate-200 dark:bg-slate-800 border border-dashed border-slate-400 dark:border-slate-700"
                  ></div>
                  <span>Blocked</span>
                </div>
                <div class="flex items-center gap-1">
              ...
              <div v-if="activeConnections.length > 0" class="space-y-1.5">
                ...
                >
                  ...
                  <div class="flex items-center gap-1.5">
                    <span
                      v-if="conn.poe && conn.poe !== 'off'"
                      class="text-[10px]"
                      :title="getPoeLabel(conn.poe)"
                      >⚡</span
                    >
                    <span
        ...
        <section v-if="metrics?.dns">
          ...
          <div class="bg-slate-50 dark:bg-slate-800/40 rounded-lg p-3 space-y-2">
            <div v-if="metrics.dns.reverse_dns" class="flex items-start justify-between gap-2">
              ...
              >
              <span
                class="text-sm font-medium text-slate-700 dark:text-slate-300 text-right break-all"
                >{{ metrics.dns.reverse_dns }}</span
              >
            </div>
            <div
              v-if="metrics.dns.resolved_hostname"
              class="flex items-start justify-between gap-2"
            >
              <span class="text-xs text-slate-500 dark:text-slate-400 flex-shrink-0">Hostname</span>
              <span
                class="text-sm font-medium text-slate-700 dark:text-slate-300 text-right break-all"
                >{{ metrics.dns.resolved_hostname }}</span
              >
            </div>
            <div v-if="metrics.dns.resolution_time_ms" class="flex items-center justify-between">
              ...
              <span class="text-sm font-medium text-slate-700 dark:text-slate-300"
                >{{ metrics.dns.resolution_time_ms.toFixed(1) }} ms</span
              >
            </div>
            <div
              v-if="
                !metrics.dns.success && !metrics.dns.reverse_dns && !metrics.dns.resolved_hostname
              "
          ...
          <div class="bg-slate-50 dark:bg-slate-800/40 rounded-lg overflow-hidden">
            ...
            >
              <div class="flex items-center gap-2">
                ...
                <span v-if="port.service" class="text-xs text-slate-500 dark:text-slate-400">{{
                  port.service
                }}</span>
              </div>
        ...
        <section>
          ...
          <div class="bg-slate-50 dark:bg-slate-800/40 rounded-lg p-3 space-y-2">
            ...
            <div v-if="metrics?.last_seen_online" class="flex items-center justify-between">
              ...
              <span class="text-sm font-medium text-slate-700 dark:text-slate-300">{{
                formatTimestamp(metrics.last_seen_online)
              }}</span>
            </div>
            <div
              v-if="metrics?.consecutive_failures && metrics.consecutive_failures > 0"
        ...
        <section>
          ...
          <div class="bg-slate-50 dark:bg-slate-800/40 rounded-lg p-3">
            ...
            <!-- No notes (readonly) -->
            <div v-else class="text-sm text-slate-400 dark:text-slate-500 italic">No notes</div>
          </div>
        </section>
        ...
        <Teleport to="body">
          ...
          >
            <div
              class="bg-white dark:bg-slate-800 rounded-xl shadow-xl w-80 max-h-[80vh] overflow-auto"
            >
              <div
                class="px-4 py-3 border-b border-slate-200 dark:border-slate-700/50 flex items-center justify-between bg-amber-50 dark:bg-amber-900/20"
              >
                <h4
                  class="font-semibold text-slate-800 dark:text-slate-100 flex items-center gap-2"
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                ...
                <button
                  ...
                  class="p-1 rounded hover:bg-amber-100 dark:hover:bg-slate-700"
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                ...
                <div v-if="editingPort.status !== 'blocked' && editingPort.type === 'rj45'">
                  ...
                  <div class="grid grid-cols-4 gap-1">
                    <button
                      ...
                      class="px-2 py-1.5 text-xs rounded transition-colors"
                      :class="
                        !editingPort.poe || editingPort.poe === 'off'
                          ? 'bg-slate-500 text-white'
                          : 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-700'
                      "
                    >
              ...
              <div
                class="px-4 py-3 border-t border-slate-200 dark:border-slate-700/50 flex justify-end gap-2"
              >
                <button
                  @click="closePortEditor"
        ...
        <Teleport to="body">
          ...
          >
            ...
            >
              <div
                class="px-4 py-3 border-b border-slate-200 dark:border-slate-700/50 flex items-center justify-between"
              >
                <h4
                  class="font-semibold text-slate-800 dark:text-slate-100 flex items-center gap-2"
                >
              ...
              <!-- Port Info Summary -->
              <div
                class="px-4 py-2 bg-slate-50 dark:bg-slate-800/40 border-b border-slate-200 dark:border-slate-700/50"
              >
                <div class="flex items-center gap-3 text-xs">
                  ...
                  <span
                    v-if="editingPort.poe && editingPort.poe !== 'off'"
                    class="flex items-center gap-0.5 text-yellow-600 dark:text-yellow-400"
                  >
              ...
              <div class="p-4 space-y-4">
                ...
                <div
                  v-if="editingPort.status === 'blocked'"
                ...
                <div v-if="editingPort.status !== 'blocked'">
                  <label
                    class="text-xs text-slate-500 dark:text-slate-400 mb-1.5 block font-medium"
                  >
                    Connection Label
              ...
              <div
                class="px-4 py-3 border-t border-slate-200 dark:border-slate-700/50 flex justify-between items-center"
              >
                <button
        ...
        <section v-if="isGateway && node?.ip">
          ...
          <div v-show="testIPsExpanded" class="space-y-3">
            ...
            <div
              v-if="testIPsError"
              class="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-2 text-xs text-red-700 dark:text-red-300"
            >
              {{ testIPsError }}
              ...
              >
                ...
                <div class="flex items-center justify-between mb-2">
                  <div class="flex items-center gap-2">
                    ...
                    <div>
                      <span
                        class="text-sm font-medium text-slate-700 dark:text-slate-300 font-mono"
                        >{{ tip.ip }}</span
                      >
                      <span v-if="tip.label" class="text-xs text-slate-500 dark:text-slate-400 ml-2"
                        >({{ tip.label }})</span
                      >
                    </div>
                  </div>
                ...
                <template v-if="getTestIPMetrics(tip.ip)">
                  <div class="space-y-2">
                      ...
                      <span
                        ...
                        :class="{
                          'text-emerald-600 dark:text-emerald-400':
                            getTestIPMetrics(tip.ip)?.status === 'healthy',
                          'text-amber-600 dark:text-amber-400':
                            getTestIPMetrics(tip.ip)?.status === 'degraded',
                          'text-red-600 dark:text-red-400':
                            getTestIPMetrics(tip.ip)?.status === 'unhealthy',
                          'text-slate-500 dark:text-slate-400':
                            !getTestIPMetrics(tip.ip)?.status ||
                            getTestIPMetrics(tip.ip)?.status === 'unknown',
                        }"
                      >
                    ...
                    <!-- Latency -->
                    <div
                      v-if="getTestIPMetrics(tip.ip)?.ping?.avg_latency_ms != null"
                    ...
                    <!-- Packet Loss -->
                    <div
                      v-if="getTestIPMetrics(tip.ip)?.ping?.packet_loss_percent != null"
                      class="flex items-center justify-between text-xs"
                    >
                      ...
                      <span
                        class="font-medium"
                        :class="
                          getTestIPMetrics(tip.ip)?.ping?.packet_loss_percent === 0
                            ? 'text-emerald-600 dark:text-emerald-400'
                            : 'text-red-600 dark:text-red-400'
                        "
                      >
                    ...
                    <!-- Jitter -->
                    <div
                      v-if="getTestIPMetrics(tip.ip)?.ping?.jitter_ms != null"
                      class="flex items-center justify-between text-xs"
                    >
                    ...
                    <!-- 24h Uptime -->
                    <div
                      v-if="getTestIPMetrics(tip.ip)?.uptime_percent_24h != null"
                      class="pt-2 border-t border-slate-200 dark:border-slate-700/50"
                    >
                      ...
                      >
                        <div
                          v-for="(entry, idx) in getTestIPMetrics(tip.ip)?.check_history?.slice(
                            -50
                          )"
                          :key="idx"
                          class="h-full"
                          :class="entry.success ? 'bg-emerald-500' : 'bg-red-500'"
                          :style="{
                            width: `${100 / Math.min(50, getTestIPMetrics(tip.ip)?.check_history?.length || 1)}%`,
                          }"
                        ></div>
                      </div>
                    </div>
                    ...
                    <div
                      v-if="
                        getTestIPMetrics(tip.ip)?.checks_passed_24h ||
                        getTestIPMetrics(tip.ip)?.checks_failed_24h
                      "
                      class="flex items-center justify-between text-xs"
                    >
                      ...
                      <span class="space-x-1">
                        <span class="text-emerald-600 dark:text-emerald-400"
                          >{{ getTestIPMetrics(tip.ip)?.checks_passed_24h || 0 }}✓</span
                        >
                        <span class="text-slate-400">/</span>
                        <span class="text-red-600 dark:text-red-400"
                          >{{ getTestIPMetrics(tip.ip)?.checks_failed_24h || 0 }}✗</span
                        >
                      </span>
                    </div>
                    ...
                    <!-- Last check time -->
                    <div
                      v-if="getTestIPMetrics(tip.ip)?.last_check"
                      class="flex items-center justify-between text-xs"
                    >
            ...
            <div
              v-else-if="!testIPsLoading"
              class="bg-slate-50 dark:bg-slate-800/40 rounded-lg p-4 text-center"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                class="h-8 w-8 mx-auto mb-2 text-slate-400 dark:text-slate-500"
              ...
              <p class="text-xs text-slate-500 dark:text-slate-400 mb-2">No test IPs configured</p>
            ...
            <div
              v-if="hasWritePermission && showAddTestIP"
              class="bg-slate-50 dark:bg-slate-800/40 rounded-lg p-3 space-y-2"
            >
              <div class="flex gap-2">
                <input
                  v-model="newTestIP"
        ...
        <section v-if="isGateway">
          ...
          <div v-show="speedTestExpanded" class="space-y-3">
            ...
            <!-- Speed test results -->
            <div
              v-if="speedTestResult && speedTestResult.success"
              class="bg-slate-50 dark:bg-slate-800/40 rounded-lg p-3 space-y-3"
            >
              ...
              <!-- Ping -->
              <div
                v-if="speedTestResult.ping_ms != null"
                class="flex items-center justify-between text-xs"
              >
                <span class="text-slate-500 dark:text-slate-400">Ping</span>
                <span class="font-medium text-slate-700 dark:text-slate-300"
                  >{{ speedTestResult.ping_ms.toFixed(1) }} ms</span
                >
              </div>

              <!-- Server info -->
              <div
                v-if="speedTestResult.server_name || speedTestResult.server_location"
                class="pt-2 border-t border-slate-200 dark:border-slate-700/50 space-y-1"
              >
                <div
                  v-if="speedTestResult.server_sponsor"
                  class="flex items-center justify-between text-xs"
                >
                  <span class="text-slate-500 dark:text-slate-400">Server</span>
                  <span class="font-medium text-slate-700 dark:text-slate-300">{{
                    speedTestResult.server_sponsor
                  }}</span>
                </div>
                <div
                  v-if="speedTestResult.server_location"
                  class="flex items-center justify-between text-xs"
                >
                  <span class="text-slate-500 dark:text-slate-400">Location</span>
                  <span class="font-medium text-slate-700 dark:text-slate-300">{{
                    speedTestResult.server_location
                  }}</span>
                </div>
              </div>
              ...
              <!-- ISP info -->
              <div
                v-if="speedTestResult.client_isp"
                class="flex items-center justify-between text-xs"
              >
                <span class="text-slate-500 dark:text-slate-400">ISP</span>
                <span class="font-medium text-slate-700 dark:text-slate-300">{{
                  speedTestResult.client_isp
                }}</span>
              </div>

              <!-- Test info -->
              <div
                class="flex items-center justify-between text-xs pt-2 border-t border-slate-200 dark:border-slate-700/50"
              >
                <span class="text-slate-500 dark:text-slate-400">Tested</span>
                <span class="text-slate-500 dark:text-slate-400">
                  {{ formatTimestamp(speedTestResult.timestamp) }}
                  <span v-if="speedTestResult.duration_seconds" class="ml-1"
                    >({{ speedTestResult.duration_seconds }}s)</span
                  >
                </span>
              </div>
            </div>
      ...
      <div v-else-if="!monitoringEnabled && node?.ip" class="p-4 space-y-4">
        ...
        <div
          v-if="!hasWritePermission"
          class="bg-slate-100 dark:bg-slate-900/50/50 border border-slate-200 dark:border-slate-700/50 rounded-lg p-2 text-xs text-slate-500 dark:text-slate-400 text-center"
        >
          View only mode
        </div>
        ...
        <section v-if="showLanPortsSection">
          ...
          <div v-show="lanPortsExpanded">
            ...
            <div
              v-if="!lanPortsConfig || lanPortsConfig.ports.length === 0"
              class="bg-slate-50 dark:bg-slate-800/40 rounded-lg p-3 text-center"
            >
              <p class="text-xs text-slate-500 dark:text-slate-400">
                {{
                  hasWritePermission
                    ? 'Enable monitoring to configure LAN ports'
                    : 'No ports configured'
                }}
              </p>
            </div>
            <div
              v-else
              class="bg-slate-100 dark:bg-slate-900/50 rounded-lg p-2 pb-3 overflow-x-auto"
            >
              <div
                class="grid gap-1.5 gap-y-2.5 min-w-fit"
                :style="{
                  gridTemplateColumns: `repeat(${lanPortsConfig.cols}, minmax(24px, 1fr))`,
                }"
              >
                <!-- RJ45 Port (SVG shape) -->
                <div
                ...
                >
                  ...
                  <template v-if="port.type === 'rj45'">
                    ...
                    <span
                      v-if="port.status !== 'blocked'"
                      class="relative z-10 text-[9px] font-mono"
                      :class="getRj45TextClasses(port)"
                      >{{ getPortLabel(port) }}</span
                    >
                    <span
                      v-else
                      class="text-slate-400 dark:text-slate-600 relative z-10 text-[9px] font-mono"
                      >✕</span
                    >
                  </template>
                  <!-- SFP/SFP+ -->
    ...
    <div
      v-if="monitoringEnabled && node?.ip && metrics && !metrics.open_ports?.length"
      class="p-3 border-t border-slate-200 dark:border-slate-700/50"
    >
      <button
        @click="scanPorts"
      ...
      >
        <svg
          xmlns="http://www.w3.org/2000/svg"
          class="h-4 w-4"
          :class="{ 'animate-spin': scanningPorts }"
          fill="none"
          viewBox="0 0 24 24"
          stroke="currentColor"
          stroke-width="2"
...
watch(
  [() => props.node?.ip, isGateway],
  async ([ip, isGw]) => {
    if (ip && isGw) {
      await Promise.all([loadTestIPs(), loadStoredSpeedTest()]);
      startTestIPPolling();
    } else {
      stopTestIPPolling();
...
async function loadTestIPs() {
  ...
  try {
    ...
    if (axiosError.response?.status === 404) {
      ...
      testIPsError.value =
        axiosError.response?.data?.detail || axiosError.message || 'Failed to load test IPs';
    }
  } finally {
...
async function saveTestIPs() {
  ...
  try {
    ...
    const axiosError = err as { response?: { data?: { detail?: string } }; message?: string };
    testIPsError.value =
      axiosError.response?.data?.detail || axiosError.message || 'Failed to save test IPs';
  } finally {
    testIPsLoading.value = false;
  }
}
...
async function checkTestIPsNow() {
  ...
  try {
    ...
    const axiosError = err as { response?: { data?: { detail?: string } }; message?: string };
    testIPsError.value =
      axiosError.response?.data?.detail || axiosError.message || 'Failed to check test IPs';
  } finally {
    testIPsLoading.value = false;
  }
}
...
async function runSpeedTest() {
  ...
  try {
    ...
    speedTestError.value =
      axiosError.response?.data?.detail || axiosError.message || 'Failed to run speed test';
  } finally {
...
const activeConnections = computed(() => {
  return sortedPorts.value.filter(
    (p) => p.status === 'active' && (p.connectedDeviceId || p.connectionLabel)
  );
});
...
function getPortClasses(port: LanPort): string {
  ...
  if (port.status === 'blocked') {
    classes.push(
      'bg-slate-200 dark:bg-slate-800 border-dashed border-slate-400 dark:border-slate-700 opacity-50'
    );
    ...
    if (port.type === 'rj45') {
      classes.push(
        'bg-amber-200 dark:bg-amber-900/50 border-amber-400 dark:border-amber-600 text-amber-800 dark:text-amber-200'
      );
    } else {
      // SFP or SFP+
      classes.push(
        'bg-cyan-200 dark:bg-cyan-900/50 border-cyan-400 dark:border-cyan-600 text-cyan-800 dark:text-cyan-200'
      );
    }
...
function getPoeLabel(poe?: PoeStatus): string {
  switch (poe) {
    case 'poe':
      return 'PoE (802.3af, 15W)';
    case 'poe+':
      return 'PoE+ (802.3at, 30W)';
    case 'poe++':
      return 'PoE++ (802.3bt, 60W+)';
    default:
      return 'No PoE';
  }
}
...
function getPoeDescription(poe?: PoeStatus): string {
  switch (poe) {
    case 'poe':
      return '802.3af - Up to 15W power delivery';
    case 'poe+':
      return '802.3at - Up to 30W power delivery';
    case 'poe++':
      return '802.3bt - Up to 60W+ power delivery';
    default:
      return 'Power over Ethernet disabled';
  }
}
...
function onConnectedDeviceChange() {
  ...
  if (editingPort.value.connectedDeviceId) {
    const device = availableDevices.value.find(
      (d) => d.id === editingPort.value!.connectedDeviceId
    );
    if (device) {
...
function savePortChanges() {
  if (!editingPort.value || !originalEditingPort.value || !lanPortsConfig.value || !props.node)
    return;
...
function canDragPort(port: LanPort): boolean {
  ...
  return (
    ...
    port.status === 'active' &&
    !!(port.connectedDeviceId || port.connectionLabel)
  );
}
...
function onPortDrop(event: DragEvent, targetPort: LanPort) {
  ...
  lanPortsConfig.value.ports[targetIndex].connectedDeviceName =
    sourceConnection.connectedDeviceName;
  ...
  lanPortsConfig.value.ports[sourceIndex].connectedDeviceName =
    targetConnection.connectedDeviceName;
  ...
  if (
    !lanPortsConfig.value.ports[sourceIndex].connectedDeviceId &&
