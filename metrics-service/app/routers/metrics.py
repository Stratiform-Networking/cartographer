...
async def get_current_snapshot(
    network_id: str | None = Query(None, description="Network ID (UUID) to get snapshot for")
):
    """
    ...
    if snapshot:
        return SnapshotResponse(success=True, snapshot=snapshot)
...
async def generate_snapshot(
    network_id: str | None = Query(None, description="Network ID (UUID) to generate snapshot for")
):
    """
...
async def publish_snapshot(
    network_id: str | None = Query(None, description="Network ID (UUID) to publish snapshot for")
):
    """
...
async def get_cached_snapshot():
    ...
    try:
        ...
        if snapshot:
            return JSONResponse({"success": True, "snapshot": snapshot.model_dump(mode="json")})

        return JSONResponse({"success": False, "message": "No cached snapshot in Redis"})
    except Exception as e:
...
async def trigger_speed_test(request: SpeedTestRequest):
    ...
    try:
        ...
        if result:
            return JSONResponse({"success": True, "result": result.model_dump(mode="json")})

        return JSONResponse(
            {"success": False, "message": "Failed to run speed test"}, status_code=500
        )
    except Exception as e:
...
async def get_summary(
    network_id: str | None = Query(None, description="Network ID (UUID) to get summary for")
):
    """
    ...
    if not snapshot:
        return JSONResponse({"available": False, "message": "No snapshot available"})
        ...
        gateways_summary.append(
            {
                ...
                "last_speed_test_timestamp": (
                    gw.last_speed_test_timestamp.isoformat()
                    if gw.last_speed_test_timestamp
                    else None
                ),
            }
        )
...
async def get_node_metrics(
    node_id: str, network_id: str | None = Query(None, description="Network ID (UUID)")
):
    ...
    if not snapshot:
        raise HTTPException(
            status_code=404, detail=f"No snapshot available for network_id={network_id}"
        )
...
async def get_connections(network_id: str | None = Query(None, description="Network ID (UUID)")):
    ...
    if not snapshot:
        return JSONResponse({"available": False, "connections": []})
...
async def get_gateways(network_id: str | None = Query(None, description="Network ID (UUID)")):
    ...
    if not snapshot:
        return JSONResponse({"available": False, "gateways": []})
...
async def reset_usage_stats(
    service: str | None = Query(None, description="Service to reset, or all if not specified")
):
    """
        ...
        return JSONResponse(
            {
                "success": success,
                "message": (
                    f"Reset usage stats for {target}" if success else "Failed to reset stats"
                ),
            }
        )
    except Exception as e:
