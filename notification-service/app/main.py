...
async def _send_cartographer_status_notification(
    ...
    try:
        ...
        if event_type == "up":
            ...
            default_message = (
                f"{downtime_str}The Cartographer monitoring service is now operational."
            )
        else:
        ...
        for subscriber in subscribers:
            ...
            if subscriber.email_enabled and subscriber.email_address and email_available:
                try:
                    ...
                    if record.success:
                        logger.info(
                            f"✓ Cartographer {event_type} email sent to {subscriber.email_address}"
                        )
                ...
                try:
                    ...
                    if discord_config:
                        record = await send_discord_notification(
                            discord_config, event, notification_id
                        )
                        if record.success:
                            logger.info(
                                f"✓ Cartographer {event_type} Discord sent to user {subscriber.user_id}"
                            )
                            subscriber_notified = True
                        else:
                            logger.error(
        ...
        logger.info(
            f"Cartographer {event_type} notification complete: {successful}/{len(subscribers)} subscribers notified"
        )
        return successful
    ...
    try:
        ...
        if not alembic_ini.exists():
                ...
                async with async_session_maker() as session:
                    try:
                        ...
                        if version_table_exists:
                            result = await session.execute(
                                text("SELECT COUNT(*) FROM alembic_version_notification")
                            )
            ...
            if needs_stamp:
                ...
                stamp_revision = (
                    "002_discord_link_context" if has_context else "001_create_notification_tables"
                )
                logger.info(
                    f"Tables exist but version table empty. Stamping database at {stamp_revision}..."
                )
