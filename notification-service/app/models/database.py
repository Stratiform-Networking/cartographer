...
class UserNetworkNotificationPrefs(Base):
    ...
    )  # FK to networks.id (UUID in main app)
    ...
    minimum_priority: Mapped[str] = mapped_column(
        PgEnum(
            "low", "medium", "high", "critical", name="notification_priority", create_type=False
        ),
        default="medium",
    )
    ...
    __table_args__ = ({"comment": "Per-user notification preferences for networks"},)
...
class UserGlobalNotificationPrefs(Base):
    ...
    __table_args__ = ({"comment": "Per-user global notification preferences"},)
...
class DiscordUserLink(Base):
    ...
    )  # network_id (UUID) for "network", null for "global"
