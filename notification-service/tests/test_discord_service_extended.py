...
class TestBuildDiscordEmbed:
    ...
    def test_build_embed_with_details(self):
        ...
        detail_fields = [f for f in embed["fields"] if f["name"] not in ["🖥️ Device", "📊 State Change", "🤖 Anomaly Detection"]]
...
class TestDiscordNotificationService:
    ...
    async def test_send_notification_not_configured(self, service):
        ...
        with patch('app.services.discord_service.DISCORD_BOT_TOKEN', ''):
            ...
            assert "not configured" in record.error_message or "not connected" in record.error_message
...
class TestConvenienceFunctions:
    ...
    async def test_send_discord_notification(self):
        ...
        with patch.object(discord_service, 'send_notification', new_callable=AsyncMock) as mock_send:
            mock_record = MagicMock()
            mock_send.return_value = mock_record
    ...
    async def test_send_test_discord(self):
        ...
        with patch.object(discord_service, 'send_test_notification', new_callable=AsyncMock) as mock_send:
